...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		 ...
		 * @var string
		 */
		protected $version = WPLMS_VERSION;//'4.1';
		...
		public function get_default_theme_style() {
			return apply_filters('wplms_default_site_style','learningcenter');
		}
		...
		public function init_globals() {
			...
			$this->features = array(
	                    		...
	                    		'bbb'=>array(
	                    						...
	                    						'verify'=>array('video-conferencing-with-bbb/video-conferencing-with-bbb.php.php','vibe-bbb/vibe-bbb.php')
                    						),
	                    		'h5p'=>array(
	                    		...
	                    		'batches'=>array(
	                    						...
	                    						'link'=>'https://wplms.ticksy.com/article-categories/wplms-batches/',
	                    						'verify'=>array('wplms-batches/loader.php')
                    						),
	                    		'attendance'=>array(
	                    		...
	                    		'appointments'=>array(
	                    						...
	                    						'description'=> 'Calendar Booking Appointments for WPLMS. <strong style="color:#57d657">Price $59</strong> | 30 Day Free Trial',
			...
			$this->site_styles = apply_filters('wplms_demo_styles',array(
		...
		public function init_wizard_steps() {
			...
			if(!empty($_GET['referrer']) && $_GET['referrer'] == 'upgrader'){
				...
				$this->steps['sync_content'] = array(
					'name'    => esc_html__( 'Syncronise Content','vibe' ),
					'view'    => array( $this, 'syncronise_content' ),
					'handler' => '',
				);

				$this->steps['functional_settings'] = array(
					'name'    => esc_html__( 'Functional settings','vibe' ),
			...
			$this->steps['next_steps']      = array(
				'name'    => esc_html__( 'Are you ready to Roll ?','vibe' ),
		...
		function save_functional_settings(){
			...
			if(empty($_POST['vibebp_login'])){
				$existing_vibe_options = get_option('wplms');
				$existing_vibe_options['vibebp_login'] = 1;
				update_option('wplms',$existing_vibe_options);
			}
			$option = get_option('vibe_customizer');
    
			if(!empty($_POST['vibebp_profiles'])){
				$option['profile_layout'] = 'blank';
			}

			if(!empty($_POST['vibebp_course_layouts'])){
				$option['course_layout'] = 'blank';	
			}

			if(!empty($_POST['vibebp_group_layouts'])){
				$option['group_layout'] = 'blank';
			}

			if(!empty($_POST['vibebp_directory_layouts'])){
				$option['directory_layout'] = 'blank';
			}
		...
		function envato_start_setup(){
            ...
            <form method="post">
                ...
                <hr><p><em>Have a suggestion for us. Share it with us <a href="https://wplms.ticksy.com/forums/forum/general/feature-request/" target="_blank">here</a>  !</em></p>
		...
		public function onboarding_step(){
			...
			if($this->step =='introduction'){
				...
				<a href="https://wplms.ticksy.com/knowledge-base/installing-wplms-version-4" target="_blank">Setup Wizard Video for Reference &rsaquo;</a>
				<?php
			}else{
		...
		public function setup_wizard() {
			...
			<span></span>
			<span>Having Troubles ? <a href="https://wplms.ticksy.com">Get Help</a></span>
			</div>';
		...
		public function envato_setup_updates_save(){
			...
			if(!empty($purchase_code)){
				$response = wp_remote_get('https://wplms.io/verify-purchase?purchase_code='.$purchase_code.'&install='.get_home_url().'&email='.get_bloginfo('admin_email'),array('timeout'     => 120));
				$body = json_decode(wp_remote_retrieve_body($response),true);
				
				if(!empty($body) && !empty($body['verify-purchase']) && !empty($body['verify-purchase']['item_id']) && $body['verify-purchase']['item_id'] == 6780226){
					update_option('wplms_purchase_code',$purchase_code);
					return true;
				}else if(!empty($body) && !empty($body['error'])){
					echo '<div class="error">'.$body['error'].'</div><a href="'.admin_url( 'themes.php?page=wplms-setup&step=updates').'" class="link">Reload</a>';
				}else{
					echo '<div class="error">Unable to validate Purchase code</div><a href="'.admin_url( 'themes.php?page=wplms-setup&step=updates').'" class="link">Reload</a>';
				}
			}
			return false;
		}
						...
						<?php 
							if(!empty($_GET['referrer']) && $_GET['referrer'] == 'upgrader'){
								...
								echo '<tr class="'.($done?'done':'').'">
										...
										<td>
											Old plugins like vibe course module,vibe customtypes ,wplms front end will bve deactivated since new wplms plugin has all of them in wplms plugin.</td>
									</tr>';
								$page_set = false;
		...
		public function envato_page_setup_save($go=null){
			...
			if(class_exists('VibeBP_SetupWizard')  && apply_filters('import_default_layouts',true)){
			...
			if(class_exists('WPLMS_4_Setup_Wizard') && apply_filters('import_default_layouts',true)){
			...
			if ( ! empty( $_REQUEST['save_step'] ) || !empty($go)){
				...
				$pages = apply_filters('import_important_page_layouts',array(
			        array(
			...
			if(!empty($_GET['referrer']) && $_GET['referrer']=='upgrader'){
				...
				if(function_exists('vibebp_get_setting') && defined('VIBE_BP_SETTINGS')){
					if(empty(vibebp_get_setting('bp_single_page'))){
						$page = array();
						$page['post_type'] = 'page';
	                    $page['post_title'] = 'App';
	                    $page['post_content'] = '[vibebp_login][vibebp_profile]';
	                    $page['post_status'] = 'publish';
	                    $id = wp_insert_post($page);
	                    if(!empty($id) && !is_wp_error($id)){
	                    	update_post_meta( $id, '_wp_page_template', 'barebones.php' );
							$all_settings = get_option(VIBE_BP_SETTINGS);
							if(empty($all_settings)){
								$all_settings = array();
							}
							if(!empty($all_settings['general'])){
								$all_settings['general']['bp_single_page'] = $id;
							}else{
			...
			$menus = vibe_get_reloaded_nav_menu();
			update_option('vibebp_reload_nav',$menus);
			flush_rewrite_rules();

			do_action('wplms_setup_wizard_page_setup');
			if(empty($go)){
				wp_redirect( esc_url_raw( $this->get_next_step_link() ) );
				exit;
			}
	...
	public function setup_wizard_header() {
		...
		public function envato_setup_introduction() {
			...
			if ( isset( $_REQUEST['debug'] ) ) {
				...
				?>
					...
					<?php 
					$class = 'no';
					if(function_exists('extension_loaded') && extension_loaded('gd')){$class = 'yes';}
					?>
					<li class="<?php echo vibe_sanitizer($class,'text'); ?>"><label><?php echo esc_html__( 'GD / IMAGEMAGIC INSTALLED','vibe'); ?></label>
					<?php
					if ( function_exists('extension_loaded') && extension_loaded('gd') ) { $check=0;
						
					} else {
						
						echo '<mark class="error">' . sprintf( __( 'ImageMagick OR GD required liraries missing from server.See: <a href="%s">BuddyPress requirements</a>', 'vibe' ), 'https://codex.buddypress.org/getting-started/' ) . '</mark>';
					}
					?>
					</li>
		...
		public function functional_settings(){
			...
			?>
			<h1><?php esc_html_e( 'Functional Settings','vibe' ); ?></h1>
			<p>Configure most important system settings on how you would like to use WPLMS.</p>
			<hr>
			<form method="post">
				...
				<div class="envato-setup-actions step">
					<div><input type="submit"
					   ...
					   value="<?php esc_html_e( 'Finalise' ,'vibe'); ?>" />
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   ><?php esc_html_e( 'Skip this step','vibe' ); ?></a>
					</div>
					<a href="<?php echo esc_url( $this->get_previous_step_link() ); ?>" class="previous_step"><?php esc_html_e( 'Previous step','vibe' ); ?></a>
		...
		public function syncronise_content(){
			?>
			<h1><?php esc_html_e( 'Syncronise Content','vibe' ); ?></h1>
			<form method="post">
					<p>WPLMS 4 is a revolutionary framework. Much faster and built for modern web.
				<hr></p>
				<table class="" cellspacing="0">
					<thead>
					<tr>
						<th class="item"><?php esc_html_e( 'Item' ,'vibe'); ?></th>
						<th class="item"><?php esc_html_e( 'Description' ,'vibe'); ?></th>
						<th class="status"><?php esc_html_e( 'Status' ,'vibe'); ?></th>
					</tr>
					</thead>
					<tbody>
				<?php
				if(function_exists('wplms_get_sync_settings')){
					...
					foreach($sync_settings as $setting){
						...
						if(!empty($setting['required_for_upgrade'])){
							echo '<tr valign="top" class="sync_step">
								...
								<td class="forminp"><a class=" sync_resync" data-id="'.$setting['id'].'">'.__('Waiting for sync ','vibe').'</a></td></tr>';
						}
					}
					wp_nonce_field('sync_resync','sync_security');
				}else{
					echo 'Wplms plugin not active';
				}
				...
				<div class="envato-setup-actions step">
					<div><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="large_next_button button-next"
					   data-callback="run_upgrader"><?php esc_html_e( 'Upgrade & Sync' ,'vibe'); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   ><?php esc_html_e( 'Skip this step','vibe' ); ?></a>
					</div>
					<a href="<?php echo esc_url( $this->get_previous_step_link() ); ?>" class="previous_step"><?php esc_html_e( 'Previous step','vibe' ); ?></a>
		...
		public function envato_setup_default_content() {
			...
			<form method="post">
				...
				<hr><p><strong>Note</strong>&nbsp;&nbsp;If you do not see "Posts", "Pages" in import items section. Make sure to reload this page. Make sure you are connected to the internet  before content re-import. <br>Try <a href="<?php echo admin_url('themes.php?page=wplms-setup&step=default_content&force=1');?>">Load content from Alternate server</a> <br>Also see <a href="https://www.youtube.com/watch?v=xXMg6VjYoWw" target="_blank">video</a> or <a href="https://wplms.ticksy.com/knowledge-base/wplms-setup-wizard-import-content-not-loading/" target="_blank">document</a> help</a></p>
		...
		public function _content_install_settings() {
			...
			$create=0;
			$main_menu = wp_get_nav_menu_object( apply_filters('wplms_import_main_menu','MainMenu'));
			if(!empty($main_menu)){
            	...
            	$vibebp_settings = array(
				    'general' => array(
			            	...
			            	'login' => [
			                    ...
			                    'login_message' => 'Sign in to experience the next generation of wplms',
			                    'login_terms' => 'To makewplmswork, we log user data and share it with service providers. Click “Sign in” above to accept VibeThemes’s Terms of Service &amp; Privacy Policy.',
	                    ...
	                    foreach ( $this->site_styles as $style_name => $style_data ) {
                            ...
                            <li<?php echo vibe_sanitizer($style_name == $current_style ? ' class="current" ' : ''); ?>>
                                <a href="#" class="sitestyle" data-style="<?php echo esc_attr( $style_name ); ?>">
                                	<span class="version_<?php echo $style_data['version']; ?>">version <?php echo $style_data['version']; ?></span><img
		...
		public function envato_setup_design_save($theme=null) {
			...
			$this->handle_demo_adjustments($demo_style);
		...
		public function envato_setup_ready() {
			...
			<div class="envato-setup-next-steps">
				...
				<div class="envato-setup-next-steps-last">
					...
					<ul>
						<li class="documentation"><a href="https://wplms.ticksy.com/"
						...
						<li class="support"><a href="https://wplms.ticksy.com/"
		...
		function handle_demo_adjustments($demo_style=null){
			if(empty($demo_style)){
				$demo_style = get_option( 'wplms_site_style' );
			}

			if(!empty($demo_style)){
				if($demo_style=='school'){
					$locations = get_theme_mod('nav_menu_locations');
					if(empty($locations['top-menu'])){
						$locations['top-menu'] = 155;
						set_theme_mod( 'nav_menu_locations', $locations );
					}
					if(defined('VIBE_BP_SETTINGS')){
						global $wpdb;
						$apppage = $wpdb->get_var("SELECT ID FROM {$wpdb->posts} WHERE post_name='app'");
						if(!empty($apppage )){
							$all_settings = get_option(VIBE_BP_SETTINGS);
							if(empty($all_settings)){
								$all_settings = array();
							...
							$all_settings['general']['general']['bp_single_page'] = $apppage;
					...
					if(function_exists('vibe_update_option')){
						global $wpdb;
						$certificate_page = $wpdb->get_var("SELECT ID FROM {$wpdb->posts} WHERE post_name='default_certificate'");
						if(!empty($certificate_page )){
							vibe_update_option('default_certificate',$certificate_page);
						}
					}
...
function vibe_get_reloaded_nav_menu(){
	$menus = array (
				  0 => 
				  array (
				    ...
				    'name' => 'Dashboard',
				    'slug' => 'dashboard',
				    'link' => 'http://localhost/wplms/members-directory/admin/dashboard/',
				    'css_id' => 'dashboard',
				    ...
				    array (
				      0 => 
				      array (
				        '__PHP_Incomplete_Class_Name' => 'VibeBP_Actions',
				        'templates' => 
				        array (
				          'templates/app.php' => 'VibeBP App',
				        ),
				      ),
				      1 => 'show_screen',
				    ),
				  ...
				  array (
				    ...
				    'name' => 'Profile',
				    'slug' => 'profile',
				    'link' => 'http://localhost/wplms/members-directory/admin/profile/',
				    'css_id' => 'xprofile',
				  ...
				  array (
				    'name' => 'Edit',
				    'link' => 'http://localhost/wplms/members-directory/admin/profile/edit/',
				    'slug' => 'edit',
				    'parent_slug' => 'profile',
				    'css_id' => 'edit',
				    'position' => 20,
				  ...
				  array (
				    ...
				    'link' => 'http://localhost/wplms/members-directory/admin/profile/change-avatar/',
				    'slug' => 'change-avatar',
				    'parent_slug' => 'profile',
				  ...
				  array (
				    ...
				    'name' => 'Activity',
				    'slug' => 'activity',
				    'link' => 'http://localhost/wplms/members-directory/admin/activity/',
				    'css_id' => 'activity',
				    'show_for_displayed_user' => true,
				    'position' => 10,
				  ...
				  array (
				    'name' => 'Mentions',
				    'link' => 'http://localhost/wplms/members-directory/admin/activity/mentions/',
				    'slug' => 'mentions',
				    'parent_slug' => 'activity',
				  ...
				  array (
				    'name' => 'Favorites',
				    'link' => 'http://localhost/wplms/members-directory/admin/activity/favorites/',
				    'slug' => 'favorites',
				    'parent_slug' => 'activity',
				    'css_id' => 'activity-favs',
				    'position' => 30,
				    'user_has_access' => true,
				    'no_access_url' => '',
				  ...
				  array (
				    'name' => 'Friends',
				    'link' => 'http://localhost/wplms/members-directory/admin/activity/friends/',
				    'slug' => 'friends',
				    'parent_slug' => 'activity',
				    'css_id' => 'activity-friends',
				    'position' => 40,
				    'user_has_access' => true,
				    'no_access_url' => '',
				  ...
				  array (
				    'name' => 'Groups',
				    'link' => 'http://localhost/wplms/members-directory/admin/activity/groups/',
				    'slug' => 'groups',
				    'parent_slug' => 'activity',
				    'css_id' => 'activity-groups',
				    'position' => 50,
				    'user_has_access' => true,
				    'no_access_url' => '',
				  ...
				  11 => 
				  array (
				    'component_id' => 'friends',
				    'name' => 'Friends <span class="count">0</span>',
				    'slug' => 'friends',
				    'link' => 'http://localhost/wplms/members-directory/admin/friends/',
				    'css_id' => 'friends',
				    'show_for_displayed_user' => true,
				    'position' => 60,
				  ...
				  array (
				    'name' => 'Requests',
				    'link' => 'http://localhost/wplms/members-directory/admin/friends/requests/',
				    'slug' => 'requests',
				    'parent_slug' => 'friends',
				    'css_id' => 'requests',
				    'position' => 20,
				  ...
				  array (
				    'component_id' => 'groups',
				    'name' => 'Groups <span class="no-count">0</span>',
				    'slug' => 'groups',
				    'link' => 'http://localhost/wplms/members-directory/admin/groups/',
				    'css_id' => 'groups',
				    'show_for_displayed_user' => true,
				    'position' => 70,
				  ...
				  array (
				    'name' => 'Invitations',
				    'link' => 'http://localhost/wplms/members-directory/admin/groups/invites/',
				    'slug' => 'invites',
				    'parent_slug' => 'groups',
				    'css_id' => 'invites',
				    'position' => 30,
				  ...
				  array (
				    ...
				    'name' => 'Messages <span class="no-count">0</span>',
				    'slug' => 'messages',
				    'link' => 'http://localhost/wplms/members-directory/admin/messages/',
				    'css_id' => 'messages',
				    'show_for_displayed_user' => true,
				    'position' => 50,
				  ...
				  array (
				    'name' => 'Starred',
				    'link' => 'http://localhost/wplms/members-directory/admin/messages/starred/',
				    'slug' => 'starred',
				    'parent_slug' => 'messages',
				    'css_id' => 'starred',
				    'position' => 11,
				  ...
				  array (
				    'name' => 'Sent',
				    'link' => 'http://localhost/wplms/members-directory/admin/messages/sentbox/',
				    'slug' => 'sentbox',
				    'parent_slug' => 'messages',
				    'css_id' => 'sentbox',
				    'position' => 20,
				  ...
				  array (
				    'name' => 'Compose',
				    'link' => 'http://localhost/wplms/members-directory/admin/messages/compose/',
				    'slug' => 'compose',
				    'parent_slug' => 'messages',
				    'css_id' => 'compose',
				    'position' => 30,
				  ...
				  array (
				    'name' => 'Notices',
				    'link' => 'http://localhost/wplms/members-directory/admin/messages/notices/',
				    'slug' => 'notices',
				    'parent_slug' => 'messages',
				    'css_id' => 'notices',
				    'position' => 90,
				  ...
				  array (
				    ...
				    'name' => 'Notifications <span class="count">0</span>',
				    'slug' => 'notifications',
				    'link' => 'http://localhost/wplms/members-directory/admin/notifications/',
				    'css_id' => 'notifications',
				    'show_for_displayed_user' => true,
				    'position' => 30,
				  ...
				  array (
				    ...
				    'css_id' => 'notifications-my-notifications',
				    'position' => 10,
				    'user_has_access' => true,
				    'no_access_url' => '',
				    'screen_function' => 'bp_notifications_screen_unread',
				  ...
				  array (
				    'name' => 'Read',
				    'link' => 'http://localhost/wplms/members-directory/admin/notifications/read/',
				    'slug' => 'read',
				    'parent_slug' => 'notifications',
				    'css_id' => 'read',
				    'position' => 20,
				  ...
				  array (
				    ...
				    'name' => 'Settings',
				    'slug' => 'settings',
				    'link' => 'http://localhost/wplms/members-directory/admin/settings/',
				    'css_id' => 'settings',
				    'show_for_displayed_user' => true,
				    'position' => 100,
				  ...
				  array (
				    'name' => 'Email',
				    'link' => 'http://localhost/wplms/members-directory/admin/settings/notifications/',
				    'slug' => 'notifications',
				    'parent_slug' => 'settings',
				    'css_id' => 'notifications',
				    'position' => 20,
				  ...
				  array (
				    'name' => 'Capabilities',
				    'link' => 'http://localhost/wplms/members-directory/admin/settings/capabilities/',
				    'slug' => 'capabilities',
				    'parent_slug' => 'settings',
				    'css_id' => 'capabilities',
				    'position' => 80,
				  ...
				  array (
				    'name' => 'Export Data',
				    'link' => 'http://localhost/wplms/members-directory/admin/settings/data/',
				    'slug' => 'data',
				    'parent_slug' => 'settings',
				    'css_id' => 'data',
				    'position' => 89,
				  ...
				  array (
				    'name' => 'Delete Account',
				    'link' => 'http://localhost/wplms/members-directory/admin/settings/delete-account/',
				    'slug' => 'delete-account',
				    'parent_slug' => 'settings',
				    'css_id' => 'delete-account',
				    'position' => 90,
				    'user_has_access' => false,
				    'no_access_url' => '',
				  ...
				  array (
				    ...
				    'link' => 'http://localhost/wplms/members-directory/admin/settings/profile/',
				    'slug' => 'profile',
				    'parent_slug' => 'settings',
				    'css_id' => 'profile',
				    'position' => 30,
				  ...
				  array (
				    ...
				    'name' => 'Courses',
				    'slug' => 'course',
				    'link' => 'http://localhost/wplms/members-directory/admin/course/',
				    'css_id' => 'course',
				    'show_for_displayed_user' => false,
				    'position' => 5,
				    'screen_function' => 'bp_course_my_courses',
				    'default_subnav_slug' => 'course',
				    'primary' => true,
				    'class' => 
				    array (
				      0 => 'menu-child',
				  ...
				  array (
				    'name' => 'Achievements',
				    'link' => 'http://localhost/wplms/members-directory/admin/course/course-stats/',
				    'slug' => 'course-stats',
				    'parent_slug' => 'course',
				    'css_id' => 'course-stats',
				    'position' => 20,
				  ...
				  array (
				    'name' => 'My Quizzes',
				    'link' => 'http://localhost/wplms/members-directory/admin/course/quiz_results/',
				    'slug' => 'quiz_results',
				    'parent_slug' => 'course',
				    'css_id' => 'quiz_results',
				    'position' => 30,
				  ...
				  array (
				    'name' => 'My Assignments',
				    'link' => 'http://localhost/wplms/members-directory/admin/course/assignment_results/',
				    'slug' => 'assignment_results',
				    'parent_slug' => 'course',
				    'css_id' => 'assignment_results',
				    'position' => 40,
				    'user_has_access' => 'read',
				    'no_access_url' => '',
				    'screen_function' => 'bp_course_my_results',
				  ...
				  array (
				    'name' => 'Notes & Reviews',
				    'link' => 'http://localhost/wplms/members-directory/admin/course/notes_reviews/',
				    'slug' => 'notes_reviews',
				    'parent_slug' => 'course',
				    'css_id' => 'notes_reviews',
				    'position' => 40,
				    'user_has_access' => 'read',
				    'no_access_url' => '',
				    'screen_function' => 'bp_course_my_results',
				  ...
				  array (
				    ...
				    'link' => 'http://localhost/wplms/members-directory/admin/course/instructor_controls/',
				    'slug' => 'instructor_controls',
				    'parent_slug' => 'course',
				    'css_id' => 'instructor_controls',
				    'position' => 150,
				  ...
				  array (
				    'name' => 'Manage Courses',
				    'link' => 'http://localhost/wplms/members-directory/admin/course/manage_courses/',
				    'slug' => 'manage_courses',
				    'parent_slug' => 'course',
				    'css_id' => 'manage_courses',
				    'position' => 160,
				    'user_has_access' => 'edit_posts',
				    'no_access_url' => '',
				    'screen_function' => 'bp_course_instructor_courses',
				  ...
				  array (
				    'name' => 'Manage Quizzes',
				    'link' => 'http://localhost/wplms/members-directory/admin/course/manage_quizzes/',
				    'slug' => 'manage_quizzes',
				    'parent_slug' => 'course',
				    'css_id' => 'manage_quizzes',
				    'position' => 170,
				    'user_has_access' => 'edit_posts',
				    'no_access_url' => '',
				    'screen_function' => 'bp_course_instructor_courses',
				  ...
				  array (
				    ...
				    'link' => 'http://localhost/wplms/members-directory/admin/course/manage_assignments/',
				    'slug' => 'manage_assignments',
				    'parent_slug' => 'course',
				    'css_id' => 'manage_assignments',
				    'position' => 180,
				    'user_has_access' => 'edit_posts',
				    'no_access_url' => '',
				    'screen_function' => 'bp_course_instructor_courses',
				  ...
				  array (
				    ...
				    'link' => 'http://localhost/wplms/members-directory/admin/course/manage_students/',
				    'slug' => 'manage_students',
				    'parent_slug' => 'course',
				    'css_id' => 'manage_students',
				    'position' => 190,
				    'user_has_access' => 'edit_posts',
				    'no_access_url' => '',
				    'screen_function' => 'bp_course_instructor_courses',
				  ...
				  array (
				    ...
				    'link' => 'http://localhost/wplms/members-directory/admin/course/manage_questions/',
				    'slug' => 'manage_questions',
				    'parent_slug' => 'course',
				    'css_id' => 'manage_questions',
				    'position' => 200,
				    'user_has_access' => 'edit_posts',
				    'no_access_url' => '',
				    'screen_function' => 'bp_course_instructor_courses',
				  ...
				  array (
				    ...
				    'link' => 'http://localhost/wplms/members-directory/admin/course/qna/',
				    'slug' => 'qna',
				    'parent_slug' => 'course',
				    'css_id' => 'qna',
				    'position' => 210,
				  ...
				  46 => 
				  array (
				    'name' => 'Manage Reports<span>Beta</span>',
				    'link' => 'http://localhost/wplms/members-directory/admin/course/manage_reports/',
				    'slug' => 'manage_reports',
				    'parent_slug' => 'course',
				    'css_id' => 'manage_reports',
				    'position' => 250,
				    'user_has_access' => true,
				    'no_access_url' => '',
				    'screen_function' => 'bp_course_instructor_courses',
				  ...
				  array (
				    'name' => 'Course',
				    'link' => 'http://localhost/wplms/members-directory/admin/activity/course/',
				    'slug' => 'course',
				    'parent_slug' => 'activity',
				    'css_id' => 'course',
				    'position' => 45,
				    ...
				    array (
				      0 => 
				      array (
				        '__PHP_Incomplete_Class_Name' => 'bp_course_activity',
				      ),
				      1 => 'bp_course_my_course_activity',
				    ),
				    'show_in_admin_bar' => false,
				    'secondary' => true,
				    'class' => 
				  ...
				  array (
				    ...
				    'name' => 'Shop',
				    'slug' => 'shop',
				    'link' => 'http://localhost/wplms/members-directory/admin/shop/',
				    'css_id' => 'shop',
				    'show_for_displayed_user' => false,
				    'position' => 55,
				    'screen_function' => 
				  ...
				  array (
				    'name' => 'Orders',
				    'link' => 'http://localhost/wplms/members-directory/admin/shop/orders/',
				    'slug' => 'orders',
				    'parent_slug' => 'shop',
				    'css_id' => 'orders',
				    'position' => 90,
				  ...
				  array (
				    'name' => 'Downloads',
				    'link' => 'http://localhost/wplms/members-directory/admin/shop/downloads/',
				    'slug' => 'downloads',
				    'parent_slug' => 'shop',
				    'css_id' => 'downloads',
				    'position' => 90,
				  ...
				  array (
				    'name' => 'Addresses',
				    'link' => 'http://localhost/wplms/members-directory/admin/shop/edit-address/',
				    'slug' => 'edit-address',
				    'parent_slug' => 'shop',
				    'css_id' => 'edit-address',
				    'position' => 90,
